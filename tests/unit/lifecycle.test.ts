...
test("pre-log failure still forwards upstream response without finalize", async () => {
  let finalizeCalls = 0;
  const { db, usageService, handle } = createHarness(async () => new Response(JSON.stringify({ ok: true }), {
    status: 200,
    headers: { "content-type": "application/json" },
  }));
  usageService.preLog = () => {
  ...
  usageService.finalizeUsage = async (id, log) => {
    finalizeCalls += 1;
    return UsageService.create(db).finalizeUsage(id, log);
  };
