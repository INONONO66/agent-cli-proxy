  ...
  export function updateFinalize(
    ...
    const stmt = db.prepare(`
      ...
      SET provider = COALESCE(?, provider),
          ...
          prompt_tokens = MAX(prompt_tokens, COALESCE(?, prompt_tokens)),
          completion_tokens = MAX(completion_tokens, COALESCE(?, completion_tokens)),
          cache_creation_tokens = MAX(cache_creation_tokens, COALESCE(?, cache_creation_tokens)),
          cache_read_tokens = MAX(cache_read_tokens, COALESCE(?, cache_read_tokens)),
          reasoning_tokens = MAX(reasoning_tokens, COALESCE(?, reasoning_tokens)),
          total_tokens = MAX(total_tokens, COALESCE(?, total_tokens)),
...
export namespace QuotaRepo {
  ...
  export function deleteOlderThanDays(db: Database, days: number): number {
